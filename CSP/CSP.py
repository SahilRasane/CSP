...
# Create the map
map1 = Basemap(llcrnrlon=-119, llcrnrlat=22, urcrnrlon=-64, urcrnrlat=49, projection='lcc', lat_1=33, lat_2=45,
               lon_0=-95)

map1.readshapefile(r'''C:\Users\rsahi\Desktop\IS PROJECT 3\st99_d00''', name='states', drawbounds=True)
state_names = []
...
def next_value_get(region, nation, nation_assignment):
    if heuristic_for_depth_first_search == 0:
        ...
        print(info, "--Sort - ()()()()()")
        if singleton == 0:
            candidates = [number for _, _, number in info]
        else:
            candidates = [number for _, _, _, number in info]
        return candidates
...
def DFS_path_calculation(region, nation, nation_assignment):
    ...
    for state_color in color_assignment_get(region, nation, nation_assignment):
        ...
        for k in next_value_get(region, nation, nation_assignment):
            if k not in painted:
...
def DFS_path_calculation_forward(region, nation, nation_assignment):
    ...
    for state_color in color_assignment_get(region, nation, nation_assignment):
        ...
        if singleton == 1 and heuristic_for_depth_first_search == 0:
            nation[region] = sorted(nation[region], key=lambda array_sort: len(nation_assignment[array_sort]),
                                    reverse=False)
        for next_neighbor in next_value_get(region, nation, nation_assignment):
...
def DrawGraph(G, painted):
    ...
    nx.draw(G, pos, with_labels=True, node_size=500, node_color=val, edge_color='black', width=1,
            alpha=.7)  # with_labels=true is to show the node number in the output graph
